...
import (
	"dbevo/commands/commandsTruncation"
	"dbevo/config"
	"dbevo/dbTesting"
...
func TestTruncationMultipleSchemas(t *testing.T) {
	// setup: connection
	postgresConfig := config.ReadConfig(e2eTesting.ConfigPathThreeDirsUp)
	...
	err := commandsTruncation.TruncationInit()
	assert.NoError(t, err)

	// when
	err = commandsTruncation.Truncate()
	...
	for _, table := range shouldBeEmptyTables {
		tableIsEmpty, tableCheckErr := dbTesting.TableIsEmpty(table)
	...
	// TODO: Log entry
	// entry is logged
	//testingUtil.EntryIsLogged(t, logEntities.CommandsDbEvoInit)
